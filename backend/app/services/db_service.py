...
Replaces every demo_data.* accessor used at runtime.

All public functions accept the same signatures as their demo_data
counterparts so call sites can be updated with minimal diff.
"""
...
async def update_equipment_sensor_values(
    ...
    Called after sensor_log / maintenance_record form submissions so that
...
async def register_equipment(
    ...
    from app.services.demo_data import _is_instrument, _infer_type
    from datetime import datetime
...
async def find_similar_incidents(
    ...
    """Return up to 3 incidents whose keyword JSON overlaps with query_keywords."""
    async with _session() as s:
        result = await s.execute(select(m.Incident))
        all_incidents = result.scalars().all()

    scored: list[tuple[int, dict[str, Any]]] = []
    for inc in all_incidents:
        kw_str = str(inc.keywords or []).lower()
        score = sum(1 for kw in query_keywords if kw.lower() in kw_str)
        if score > 0:
            d = _row_to_dict(inc)
            if d.get("extra"):
                d.update(d.pop("extra"))
...
async def get_equipment_documents(equipment_id: str) -> list[dict[str, Any]]:
...
async def upsert_document(data: dict[str, Any]) -> None:
    await save_document(data)
...
async def get_equipment_technicians(equipment_id: str) -> list[dict[str, Any]]:
    ...
    async with _session() as s:
        ...
        for row in up_rows:
            ...
            # and if no equipment filter OR their plant_ids overlap with any
            # plant containing this equipment (best-effort; always include when
            # equipment_id has no plant context).
            uid = f"USR-{row.id}"
    ...
    async with _session() as s:
        links_result = await s.execute(
            select(m.GraphLink).where(
                or_(m.GraphLink.source == equipment_id, m.GraphLink.target == equipment_id)
            )
        )
        links = links_result.scalars().all()

    relevant_ids = {equipment_id}
    for lnk in links:
        relevant_ids.add(lnk.source)
        relevant_ids.add(lnk.target)

    async with _session() as s:
        nodes_result = await s.execute(
            select(m.GraphNode).where(m.GraphNode.id.in_(relevant_ids))
        )
        full_links_result = await s.execute(
            select(m.GraphLink).where(
                m.GraphLink.source.in_(relevant_ids),
                m.GraphLink.target.in_(relevant_ids),
            )
        )
        nodes = [_row_to_dict(n) for n in nodes_result.scalars().all()]
        link_dicts = [_row_to_dict(l) for l in full_links_result.scalars().all()]
...
async def update_graph_with_document(doc_id: str, filename: str, entities: dict[str, Any]) -> None:
    """
    Add the uploaded document as a graph node and link it to any equipment IDs
    found during entity extraction.  Auto-registers unknown equipment tags.
    """
    await upsert_graph_node({"id": doc_id, "name": filename[:30], "type": "document", "val": 10})

    for eq_id in entities.get("equipment_ids", []):
        try:
            # Auto-register new equipment
...
# ─────────────────────────────────────────────────────────────────────────────
# Project Management upsert helpers (used by doc entity mapper)
# ─────────────────────────────────────────────────────────────────────────────

async def get_project_by_code(code: str) -> dict[str, Any] | None:
    async with _session() as s:
        result = await s.execute(select(m.Project).where(m.Project.code == code))
        row = result.scalar_one_or_none()
        return _row_to_dict(row) if row else None


async def upsert_project(data: dict[str, Any]) -> str:
    """Insert or update a Project row. Returns the project id."""
    import uuid as _uuid
    async with _session() as s:
        proj_id = data.get("id")
        existing = await s.get(m.Project, proj_id) if proj_id else None
        if existing is None and data.get("code"):
            result = await s.execute(select(m.Project).where(m.Project.code == data["code"]))
            existing = result.scalar_one_or_none()
        if existing:
            for k, v in data.items():
            ...
            await s.commit()
            return existing.id
        proj_id = proj_id or f"PROJ-{_uuid.uuid4().hex[:8].upper()}"
        data.setdefault("code", proj_id)
        row = m.Project(id=proj_id, **{k: v for k, v in data.items() if k != "id" and hasattr(m.Project, k)})
        s.add(row)
        await s.commit()
        return proj_id


async def get_plant_by_code(code: str) -> dict[str, Any] | None:
    async with _session() as s:
        result = await s.execute(select(m.Plant).where(m.Plant.code == code))
        row = result.scalar_one_or_none()
        return _row_to_dict(row) if row else None


async def upsert_plant(data: dict[str, Any]) -> str:
    """Insert or update a Plant row. Returns the plant id."""
    import uuid as _uuid
    async with _session() as s:
        plant_id = data.get("id")
        existing = await s.get(m.Plant, plant_id) if plant_id else None
        if existing is None and data.get("code"):
            result = await s.execute(select(m.Plant).where(m.Plant.code == data["code"]))
            existing = result.scalar_one_or_none()
        if existing:
            for k, v in data.items():
                if hasattr(existing, k) and v is not None:
            ...
            await s.commit()
            return existing.id
        plant_id = plant_id or f"PLANT-{_uuid.uuid4().hex[:8].upper()}"
        data.setdefault("code", plant_id)
        row = m.Plant(id=plant_id, **{k: v for k, v in data.items() if k != "id" and hasattr(m.Plant, k)})
        s.add(row)
        await s.commit()
        return plant_id


async def upsert_managed_work_order(data: dict[str, Any]) -> str:
    """Insert or update a ManagedWorkOrder row. Returns the wo id."""
    import uuid as _uuid
    async with _session() as s:
        wo_id = data.get("id")
        existing = await s.get(m.ManagedWorkOrder, wo_id) if wo_id else None
        if existing is None and data.get("wo_number"):
            result = await s.execute(
                select(m.ManagedWorkOrder).where(m.ManagedWorkOrder.wo_number == data["wo_number"])
            )
            existing = result.scalar_one_or_none()
        if existing:
            for k, v in data.items():
            ...
            await s.commit()
            return existing.id
        wo_id = wo_id or f"MWO-{_uuid.uuid4().hex[:8].upper()}"
        data.setdefault("wo_number", wo_id)
        data.setdefault("title", data.get("description", "Work Order from document")[:120])
        row = m.ManagedWorkOrder(
            id=wo_id,
            **{k: v for k, v in data.items() if k != "id" and hasattr(m.ManagedWorkOrder, k)},
        )
        s.add(row)
        await s.commit()
        return wo_id
...
async def upsert_incident_report(data: dict[str, Any]) -> str:
    """Insert or update an IncidentReport row. Returns the incident id."""
    import uuid as _uuid
    async with _session() as s:
        inc_id = data.get("id")
        existing = await s.get(m.IncidentReport, inc_id) if inc_id else None
        if existing is None and data.get("incident_number"):
            result = await s.execute(
                select(m.IncidentReport).where(m.IncidentReport.incident_number == data["incident_number"])
            )
            existing = result.scalar_one_or_none()
        if existing:
            for k, v in data.items():
            ...
            await s.commit()
            return existing.id
        inc_id = inc_id or f"IR-{_uuid.uuid4().hex[:8].upper()}"
        data.setdefault("incident_number", inc_id)
        data.setdefault("title", data.get("description", "Incident from document")[:120])
        row = m.IncidentReport(
            id=inc_id,
            **{k: v for k, v in data.items() if k != "id" and hasattr(m.IncidentReport, k)},
        )
        s.add(row)
        await s.commit()
        return inc_id
...
async def upsert_safety_procedure(data: dict[str, Any]) -> str:
    """Insert or update a SafetyProcedure row. Returns the procedure id."""
    import uuid as _uuid
    async with _session() as s:
        sp_id = data.get("id")
        existing = await s.get(m.SafetyProcedure, sp_id) if sp_id else None
        if existing is None and data.get("code"):
            result = await s.execute(
                select(m.SafetyProcedure).where(m.SafetyProcedure.code == data["code"])
            )
            existing = result.scalar_one_or_none()
        if existing:
            for k, v in data.items():
            ...
            await s.commit()
            return existing.id
        sp_id = sp_id or f"SP-{_uuid.uuid4().hex[:8].upper()}"
        data.setdefault("code", sp_id)
        data.setdefault("title", data.get("code", "Safety Procedure from document"))
        row = m.SafetyProcedure(
            id=sp_id,
            **{k: v for k, v in data.items() if k != "id" and hasattr(m.SafetyProcedure, k)},
        )
        s.add(row)
        await s.commit()
        return sp_id
...
async def upsert_quality_inspection(data: dict[str, Any]) -> str:
    """Insert or update a QualityInspection row. Returns the inspection id."""
    import uuid as _uuid
    async with _session() as s:
        qi_id = data.get("id")
        existing = await s.get(m.QualityInspection, qi_id) if qi_id else None
        if existing is None and data.get("inspection_number"):
            result = await s.execute(
                select(m.QualityInspection).where(
                    m.QualityInspection.inspection_number == data["inspection_number"]
                )
            )
            existing = result.scalar_one_or_none()
        if existing:
            ...
            await s.commit()
            return existing.id
        qi_id = qi_id or f"QI-{_uuid.uuid4().hex[:8].upper()}"
        data.setdefault("inspection_number", qi_id)
        data.setdefault("title", data.get("inspection_number", "Inspection from document"))
        row = m.QualityInspection(
            id=qi_id,
            **{k: v for k, v in data.items() if k != "id" and hasattr(m.QualityInspection, k)},
        )
        s.add(row)
        await s.commit()
        return qi_id
