"""
AI Operations Brain — Demo Document Generator
...
def _pdf_header(pdf, title: str):
    ...
    pdf.set_xy(8, 4)
    pdf.cell(0, 10, _s(f"AI OPERATIONS BRAIN  |  {title}"))
...
def _gen_pdf_maintenance_report() -> bytes:
    ...
    _pdf_kv(pdf, "PTW Number:", "PTW-2026-018 (Cold Work — Issued 22 Jul 2026 02:41)")
    _pdf_kv(pdf, "Lead Technician:", "Rajesh Kumar (EMP-001) — HAZOP, PTW-Technician certified")
    ...
    pdf.multi_cell(0, 5, _s(
        ...
        "under PTW-0042 (March 2026). Misalignment measurement today was within limits (0.3 mrad) — likely not primary.\n\n"
...
def _gen_pdf_incident_report() -> bytes:
    ...
    _pdf_kv(pdf, "Status:", "CAPA Stage — Corrective actions in progress")
    ...
    pdf.multi_cell(0, 5, _s(
        ...
        "following the 2024 interval reduction (INC-2024-015 CAPA item CAPA-K401-2 — STATUS: STILL OPEN)."
    ))
    ...
    five_why = [
        ...
        ("Why 4", "CMMS not updated?", "CAPA-K401-2 (from INC-2024-015) to reduce interval to 60 days was never closed"),
        ("Why 5", "CAPA was not closed?", "No owner assigned to CAPA tracking. No automated overdue reminder in place"),
    ]
    ...
    factors = [
        "Open CAPA CAPA-K401-2 not tracked — no automated overdue reminder in CMMS",
    ...
    _pdf_section(pdf, "CAPA ITEMS")
    capa = [
        ("CAPA-K401-1", "Corrective", "Replace K-401 inlet filter — immediate", "Ahmed Khan", "CLOSED", _GREEN),
        ("CAPA-K401-2", "Preventive", "Update CMMS: filter interval to 60 days. Raise WO-2026-K401-PM", "Ahmed Khan", "OPEN", _AMBER),
        ("CAPA-K401-3", "Preventive", "Install dP transmitter on K-401 inlet — raise CAPEX request", "Deepa Menon", "OPEN", _AMBER),
        ("CAPA-K401-4", "Improvement", "Implement CAPA tracking review in monthly safety meeting agenda", "Priya Nair", "OPEN", _BLUE),
    ]
...
def _gen_docx_sop() -> bytes:
    ...
    tools = [
        ...
        "PTW form (Cold Work) — Safety Management System",
    ...
    steps = [
        ("PREPARATION", [
            ("4.1", "Obtain approved PTW (Cold Work) from Area Authority. Confirm permit is displayed at pump.",
        ...
        ("COMMISSIONING", [
            ...
            ("4.18", "Close PTW. Return all tools to Tool Room. Dispose of spent bearing and grease per waste SOP.",
...
def _gen_docx_handover() -> bytes:
    ...
    statuses = [
        ...
        ("K-401", "Running", "Discharge 10.8 bar (low)", "CAPA-K401-2 filter interval to be updated"),
    ...
    events = [
        ...
        ("02:35", "PTW-2026-018 (Cold Work — P-101 bearing replacement) created."),
        ("02:41", "PTW issued after Area Authority and Safety review. Rajesh Kumar briefed."),
        ...
        ("05:45", "P-101 returned to service. Vibration 4.1 mm/s (Normal). PTW closed."),
    ...
    pending = [
        ...
        "Follow up CAPA-K401-2 (K-401 filter interval change to 60 days) — assign owner.",
...
def _gen_pptx_toolbox_talk() -> bytes:
    ...
    run3 = p3.add_run()
    run3.text = "Key Lesson This Month: P-101 vibration alarm at 02:15 — caught before failure by OpsBrain AI."
    ...
    add_bullet_box(slide2, 0.5, 1.8, 5.8, 5.2, "THE INCIDENT (02:15 AM):", [
        ...
        ("OpsBrain AI flagged 75% bearing failure risk", AMBER),
        ("WO raised, PTW issued, bearing replaced by 05:45", GREEN),
    ...
    slide5 = add_slide("EMERGENCY CONTACTS & KEY INFORMATION",
                       "If you see a problem — speak up. OpsBrain is a tool, not a replacement for your eyes")
    ...
    add_bullet_box(slide5, 7.0, 1.8, 6.0, 5.0, "THIS MONTH'S SAFETY STATS:", [
        ...
        ("Open PTWs Today: 1 (P-101 bearing — CLOSED)", GREEN),
        ("Open Work Orders: 4 (1 critical — P-101 closed)", AMBER),
        ("Compliance Score: 78% (P-101 driving)", AMBER),
        ("CAPA Overdue: 1 (CAPA-K401-2 — assign owner!)", RED),
    ])
...
K-401  Process Air Compressor: Running. Discharge 10.8 bar (low — CAPA pending).
...
02:35  PTW-2026-018 (Cold Work) created: bearing inspection + lube oil change.
02:38  Area Authority review: Suresh Patel approved.
02:39  Safety review: Priya Nair approved.
02:41  PTW-2026-018 ISSUED. Rajesh Kumar briefed on scope and isolations.
...
3. [Ahmed Khan] CAPA-K401-2 (K-401 filter interval 60 days) — assign owner and close.
...
6. [All] OEM maintenance PDF for P-101 bearing job to be uploaded to OpsBrain.
...
def generate_demo_documents() -> list[dict]:
    ...
    def _save(doc_id: str, ext: str, content: bytes | str) -> str:
    ...
    try:
        ...
        path = _save("DOC-DEMO-PDF-01", ".pdf", content)
        records.append({
            "id": "DOC-DEMO-PDF-01",
    ...
    try:
        ...
        path = _save("DOC-DEMO-PDF-02", ".pdf", content)
        records.append({
            "id": "DOC-DEMO-PDF-02",
            ...
            "pipeline_steps": _PIPELINE_DONE,
            "sections": {
                "summary": "K-401 compressor discharge pressure dropped to 10.8 bar (rated 12.5 bar) due to choked inlet filter. CAPA-K401-2 overdue. 4h downtime, USD 2,200 cost.",
                "root_cause": "Inlet filter element completely choked. Filter change interval in CMMS set to 90 days (not updated to 60 days per CAPA-K401-2 from INC-2024-015).",
                "capa": "CAPA-K401-1: Filter replaced (CLOSED). CAPA-K401-2: CMMS interval update (OPEN). CAPA-K401-3: Install dP transmitter (OPEN).",
            },
            "entities": {
                "equipment_ids": ["K-401"],
                "parts": ["AC-ZH350-FE"],
                ...
                "capa_items": ["CAPA-K401-1", "CAPA-K401-2", "CAPA-K401-3"],
            },
        })
    except Exception as e:
    ...
    try:
        ...
        path = _save("DOC-DEMO-DOCX-01", ".docx", content)
        records.append({
            "id": "DOC-DEMO-DOCX-01",
    ...
    try:
        ...
        path = _save("DOC-DEMO-DOCX-02", ".docx", content)
        records.append({
            "id": "DOC-DEMO-DOCX-02",
            ...
            "pipeline_steps": _PIPELINE_DONE,
            "sections": {
                "summary": "Night shift handover 22 Jul 2026. P-101 vibration alarm at 02:15 — bearing replaced, back in service 05:45. K-401 discharge pressure low — CAPA pending. All other equipment running normally.",
                "events": "02:15 alarm, 02:18 AI query, 02:41 PTW issued, 03:55 bearing removal, 05:45 P-101 back in service, 06:00 handover.",
                "actions": "Close CI-P101-3, update CMMS lube interval, assign CAPA-K401-2 owner, book G-101 blade inspection.",
            },
            "entities": {
                "equipment_ids": ["P-101", "P-202", "HX-201", "V-301", "K-401", "G-101"],
    ...
    try:
        ...
        path = _save("DOC-DEMO-XLSX-01", ".xlsx", content)
        records.append({
            "id": "DOC-DEMO-XLSX-01",
    ...
    try:
        ...
        path = _save("DOC-DEMO-XLSX-02", ".xlsx", content)
        records.append({
            "id": "DOC-DEMO-XLSX-02",
    ...
    try:
        ...
        path = _save("DOC-DEMO-PPTX-01", ".pptx", content)
        records.append({
            "id": "DOC-DEMO-PPTX-01",
            ...
            "pipeline_steps": _PIPELINE_DONE,
            "sections": {
                "summary": "Monthly safety toolbox talk: rotating equipment vibration awareness and LOTO procedure. Key lesson: P-101 vibration alarm this month — caught before failure by OpsBrain AI. ISO 10816-7 vibration zones explained.",
    ...
    try:
        ...
        path = _save("DOC-DEMO-TXT-01", ".txt", content)
        records.append({
            "id": "DOC-DEMO-TXT-01",
    ...
    try:
        ...
        path = _save("DOC-DEMO-CSV-01", ".csv", content)
        lines = content.count("\n")
        records.append({
            "id": "DOC-DEMO-CSV-01",
