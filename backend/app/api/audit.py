...
Exposes the append-only audit trail with filtering, full-text search,
...
# ── CSV export ─────────────────────────────────────────────────────────────────
# NOTE: Must be registered BEFORE /{object_type}/{object_id} — FastAPI matches
# routes in declaration order, and the generic route would otherwise capture
# /export/csv as object_type="export", object_id="csv".
...
async def export_audit_csv(
    ...
    since = datetime.utcnow() - timedelta(days=days)
    stmt = select(m.AuditLog).where(m.AuditLog.timestamp >= since)
    if object_type:
        stmt = stmt.where(m.AuditLog.object_type == object_type)
    if equipment_id:
        stmt = stmt.where(m.AuditLog.equipment_id == equipment_id)
    if action:
        stmt = stmt.where(m.AuditLog.action == action)
    stmt = stmt.order_by(desc(m.AuditLog.timestamp)).limit(5000)
    ...
    for r in rows:
        writer.writerow([
            ...
            _csv_safe(r.object_type),
            _csv_safe(r.object_id),
            _csv_safe(r.equipment_id or ""),
            _csv_safe(r.action),
            _csv_safe(r.actor),
            _csv_safe(r.actor_type),
            _csv_safe(r.risk_level or ""),
            _csv_safe(r.notes or ""),
        ])
...
# ── Single object history (defined AFTER /relations/ and /export/csv) ──────────
