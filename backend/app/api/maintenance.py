...
@router.post("")
async def create_maintenance_record(body: MaintenanceCreate):
...
async def update_maintenance_record(record_id: str, body: MaintenanceUpdate):
    async with AsyncSessionLocal() as s:
        row = await s.get(m.MaintenanceRecord, record_id)
        if not row:
            raise HTTPException(status_code=404, detail="Maintenance record not found")
        ...
        if body.technician is not None:
            row.technician = body.technician
