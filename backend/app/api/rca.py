...
Phase 3 of the IndustrialGPT vision: investigates a production event and returns
a ranked causal chain with probability scores.
...
# Severity normalisation (IncidentReport P-levels → display labels)
_P_TO_SEV = {"P1": "Critical", "P2": "Critical", "P3": "High", "P4": "Medium", "P5": "Low"}
...
async def _collect_evidence(equipment_id: str | None) -> dict[str, Any]:
    ...
    evidence: dict[str, Any] = {"sensor_anomalies": [], "overdue_maintenance": [], "recent_incidents": []}
        ...
        for sensor_name, reading in readings.items():
            ...
            if val is not None and alarm is not None and val > alarm:
                anomalies.append({
                    ...
                    "alarm": alarm,
                    "excess_pct": round((val - alarm) / alarm * 100, 1),
                })
...
@router.post("/analyze")
async def analyze_root_cause(body: RCARequest) -> RCAResponse:
    """
...
async def list_quick_events():
    ...
    Return quick-start RCA events built LIVE from the current DB state:
      ...
      2. Open incident reports  — P1/P2/P3 severity, not closed
      3. Overdue maintenance    — equipment.maintenance_due_days <= 0
      4. Open high-priority WOs — ManagedWorkOrder with high/critical priority
    ...
    for eq in all_eq:
        ...
        for key, r in readings.items():
            ...
            val = r.get("value")
            alarm = r.get("alarm")
            if val is None or alarm is None or val <= alarm:
                continue
            trip = r.get("trip")
            excess_pct = round((val - alarm) / alarm * 100)
            label = key.replace("_", " ").title()
            sev = "Critical" if (trip and val > trip) else "High"
            events.append({
                "id": f"alarm-{eq['id']}-{key}",
                "title": f"{eq['id']}: {label} alarm active",
                "description": (
                    f"{eq.get('name', eq['id'])} has an active {label} alarm. "
                    f"Current reading {val} {r.get('unit', '')} exceeds alarm threshold "
    ...
    # 2. Open IncidentReports (P1-P3) ────────────────────────────────────────
    async with AsyncSessionLocal() as s:
        ir_rows = (await s.execute(
            select(m.IncidentReport)
            .where(m.IncidentReport.status.notin_(["closed"]))
            .where(m.IncidentReport.severity.in_(["P1", "P2", "P3"]))
            .order_by(m.IncidentReport.created_at.desc())
        )).scalars().all()
    for r in ir_rows[:4]:
        eq_id = (r.equipment_ids or [None])[0]
        events.append({
            "id": f"ir-{r.id}",
            "title": r.title[:70],
            "description": (r.description or r.title)[:200],
            "equipment_hint": eq_id,
            "severity": _P_TO_SEV.get(r.severity or "", "High"),
        })
    ...
    # 4. Open high/critical ManagedWorkOrders ────────────────────────────────
    async with AsyncSessionLocal() as s:
        mwo_rows = (await s.execute(
            select(m.ManagedWorkOrder)
            .where(m.ManagedWorkOrder.status.in_(
                ["draft", "submitted", "pending_approval", "approved", "scheduled", "in_progress"]
            ))
            .where(m.ManagedWorkOrder.priority.in_(["high", "critical"]))
            .order_by(m.ManagedWorkOrder.created_at.desc())
        )).scalars().all()
    for w in mwo_rows[:3]:
        eq_id = (w.equipment_ids or [None])[0]
        events.append({
            "id": f"wo-{w.id}",
            "title": f"Open WO ({w.priority}): {w.title[:50]}",
            "description": (w.description or w.title)[:200],
            "equipment_hint": eq_id,
            "severity": "Critical" if w.priority == "critical" else "High",
        })
