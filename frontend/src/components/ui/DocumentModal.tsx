/**
 * AI Operations Brain — Document Modal
...
export function DocumentModal({ docId, onClose }: DocumentModalProps) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetch(`${BASE}/api/v1/documents/${encodeURIComponent(docId)}`)
  ...
  return (
    <>
      ...
      {/* Slide-over panel */}
      <div className="fixed inset-y-0 right-0 w-full max-w-2xl bg-[#0f0f0f] border-l border-[#2a2a2a] z-50 flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-[#2a2a2a] flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            {loading ? (
              ...
              <p className="text-sm font-semibold text-[#f9f9f9] truncate">{doc?.name ?? docId}</p>
            )}
            {doc && (
