...
import { getFullGraph, traverseGraph } from "@/lib/api";
...
export function GraphView() {
  ...
  const handleNodeClick = useCallback((node: GraphNode) => {
    ...
    setSelectedNode(prev => {
      ...
      if (n.type === "equipment") {
        traverseGraph(n.id, 2)
          .then(sub => {
            if (!sub?.nodes?.length) return;
            setGraphData(prev => {
              const existingIds = new Set(prev.nodes.map(nd => nd.id));
              const newNodes = sub.nodes.filter(nd => !existingIds.has(nd.id));
        ...
        {(
          ...
          return (
            <div key={type} className="flex items-center justify-between mb-1.5">
              ...
              {graphData.nodes.length > 0 && (
                <span className="text-[11px] text-[#6b7280] font-mono">{count}</span>
              )}
            </div>
          );
        })}
