 ...
 * Shows all equipment organised by Project → Plant hierarchy.
 * Links to individual equipment detail pages.
 */
import Link from "next/link";
import { getProjectHierarchy, listEquipment } from "@/lib/api";
import { Badge } from "@/components/ui/Badge";
import { AlertTriangle, BrainCircuit, Activity, HardDrive, Building2, FolderKanban } from "lucide-react";
import type { Equipment } from "@/lib/types";
import type { HierarchyEquipment } from "@/lib/api";
...
function EquipmentRow({ eq }: { eq: HierarchyEquipment }) {
...
export default async function EquipmentPage() {
  const [hierarchy, allEquipment] = await Promise.all([
    getProjectHierarchy().catch(() => ({ projects: [], unassigned_equipment: [] })),
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-[#6b7280] mt-0.5">
            {allEquipment.length} assets across {hierarchy.projects.length} projects
          </p>
        </div>
      ...
      {/* Project → Plant → Equipment tree */}
      {hierarchy.projects.map(project => (
        <div key={project.id} className="mb-6">
          {/* Project header */}
          <div className="flex items-center gap-2 mb-3">
            <FolderKanban size={14} className="text-amber-400/70 flex-shrink-0" />
            <Link
              href={`/projects`}
              className="text-sm font-semibold text-[#e0e0e0] hover:text-amber-400 transition-colors"
            >
              {project.name}
            </Link>
            <span className="text-[10px] font-mono text-[#4b5563]">{project.code}</span>
            <Badge variant={project.status === "Active" ? "low" : "muted"}>
              {project.status}
            </Badge>
          </div>

          {project.plants.length === 0 ? (
            <p className="text-xs text-[#4b5563] ml-5 mb-2">No plants assigned</p>
          ) : (
            <div className="space-y-3 ml-4">
              {project.plants.map(plant => (
                <div key={plant.id} className="border border-[#1e1e1e] rounded-xl overflow-hidden">
                  {/* Plant header */}
                  <div className="flex items-center gap-2 px-4 py-2 bg-[#0e0e0e] border-b border-[#1e1e1e]">
                    <Building2 size={12} className="text-blue-400/70 flex-shrink-0" />
                    <span className="text-xs font-medium text-[#c0c0c0]">{plant.name}</span>
                    <span className="text-[10px] font-mono text-[#4b5563]">{plant.code}</span>
                    <span className="ml-auto text-[10px] text-[#4b5563]">
                      {plant.equipment.length} equipment
                    </span>
                  </div>
                  {/* Equipment rows */}
                  {plant.equipment.length === 0 ? (
                    <p className="px-4 py-3 text-xs text-[#4b5563]">No equipment assigned</p>
                  ) : (
                    plant.equipment.map(eq => <EquipmentRow key={eq.id} eq={eq} />)
                  )}
                </div>
              ))}
            </div>
      ...
      {hierarchy.unassigned_equipment.length > 0 && (
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-3">
            <HardDrive size={14} className="text-[#4b5563] flex-shrink-0" />
            <span className="text-sm font-semibold text-[#6b7280]">Unassigned Equipment</span>
            <span className="text-[10px] text-[#4b5563]">({hierarchy.unassigned_equipment.length})</span>
          </div>
          <div className="border border-[#1e1e1e] rounded-xl overflow-hidden">
            {hierarchy.unassigned_equipment.map(eq => <EquipmentRow key={eq.id} eq={eq} />)}
          </div>
        </div>
      )}

      {/* Fallback: no hierarchy data */}
      {hierarchy.projects.length === 0 && hierarchy.unassigned_equipment.length === 0 && (
