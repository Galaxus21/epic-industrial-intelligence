 ...
 * Shows source badge (CMMS vs Slack/AI-extracted) and status pill.
 */
"use client";
...
const ALL_STATUSES = ["Completed", "Pending", "In Progress", "Overdue", "Cancelled"];
const ALL_TYPES    = ["Preventive", "Corrective", "Predictive", "Emergency"];

function Pill({ label, cls }: { label: string; cls: string }) {
  return (
    <span className={clsx("inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded border font-medium", cls)}>
      {STATUS_ICON[label] ?? null}
      {label}
    </span>
  );
}

function SourceBadge({ record }: { record: MaintenanceRecord }) {
  const isSlack = record.source === "slack";
  const isAI = record.ai_extracted;
  if (isSlack && isAI)
    return <span className="text-[10px] px-1.5 py-0.5 rounded bg-violet-500/10 border border-violet-500/30 text-violet-400 font-medium">⚡ Slack · AI</span>;
  if (isSlack)
    return <span className="text-[10px] px-1.5 py-0.5 rounded bg-sky-500/10 border border-sky-500/30 text-sky-400 font-medium">Slack</span>;
  return null;
}
...
export default function MaintenancePage() {
  ...
  const aiParsed   = records.filter((r) => r.ai_extracted).length;
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <div className="grid grid-cols-4 gap-2 mb-4">
          {[
            ...
            { label: "AI-Parsed",   value: aiParsed,   color: "text-violet-400",  bg: "bg-violet-500/10" },
          ].map((s) => (
              ...
              <tbody>
                {filtered.map((r) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5">
                      <Pill label={r.type} cls={TYPE_STYLE[r.type] ?? "text-zinc-400 bg-zinc-700 border-zinc-600"} />
                    </td>
                    ...
                    <td className="px-3 py-2.5">
                      <Pill label={r.status} cls={STATUS_STYLE[r.status] ?? "text-zinc-400 bg-zinc-700 border-zinc-600"} />
                    </td>
                    <td className="px-3 py-2.5">
        ...
        <div className="w-80 flex-shrink-0 rounded-lg border border-zinc-800 bg-zinc-900 overflow-y-auto">
          ...
          <div className="p-4 space-y-4 text-sm">
            ...
            <div className="flex flex-wrap gap-2">
              <Pill label={selected.status} cls={STATUS_STYLE[selected.status] ?? "text-zinc-400 bg-zinc-700 border-zinc-600"} />
              <Pill label={selected.type} cls={TYPE_STYLE[selected.type] ?? "text-zinc-400 bg-zinc-700 border-zinc-600"} />
              <SourceBadge record={selected} />
            </div>
            ...
            {selected.ai_extracted && (
              <div className="p-3 rounded bg-violet-500/10 border border-violet-500/20 text-xs text-violet-300">
                ⚡ This record was automatically created from a Slack message using AI extraction.
              </div>
            )}
          </div>
        </div>
