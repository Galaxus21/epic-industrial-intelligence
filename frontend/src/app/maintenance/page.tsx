...
const STATUS_STYLE: Record<string, string> = {
  ...
  Cancelled:    "text-[#a0a0a0] bg-zinc-500/10 border-zinc-500/30",
};
...
const ALL_TYPES    = ["Preventive", "Corrective", "Predictive", "Emergency", "Inspection"];
...
export default function MaintenancePage() {
  ...
  return (
    <div className="flex h-full gap-4 p-4 overflow-hidden">
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-semibold text-[#f9f9f9]">Maintenance Logs</h1>
            <span className="text-xs text-[#a0a0a0] ml-1">{filtered.length} records</span>
          </div>
          <button
            onClick={load}
            className="p-1.5 rounded hover:bg-white/5 text-[#a0a0a0] hover:text-[#f9f9f9] transition-colors"
            title="Refresh"
          >
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          <div className="relative flex-1 min-w-40">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 text-sm bg-[#242424] border border-[#333] rounded text-[#f9f9f9] placeholder-[#4b5563] focus:outline-none focus:border-amber-400/50"
            />
          </div>
          ...
          <select
            ...
            className="text-sm bg-[#242424] border border-[#333] rounded px-2 py-1.5 text-[#d0d0d0] focus:outline-none"
          >
        ...
        {/* Table */}
        <div className="flex-1 overflow-y-auto rounded-lg border border-[#2a2a2a]">
          {loading ? (
            <div className="flex items-center justify-center h-40 text-[#6b7280] text-sm">
              <RefreshCw className="w-4 h-4 animate-spin mr-2" /> Loading…
            </div>
          ) : filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-40 text-[#6b7280] text-sm gap-2">
              ...
              <thead className="sticky top-0 bg-[#1a1a1a] border-b border-[#2a2a2a]">
                <tr>
                  {["Equipment", "Type", "Description", "Date", "Technician", "Status", ""].map((h) => (
                    <th key={h} className="px-3 py-2.5 text-left text-xs text-[#6b7280] font-medium">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map((r) => (
                  <tr
                    ...
                    className={clsx(
                      "border-b border-[#2a2a2a] hover:bg-white/3 cursor-pointer transition-colors",
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5">
                      <Pill label={r.type} cls={TYPE_STYLE[r.type] ?? "text-[#a0a0a0] bg-[#333] border-[#444]"} icon={STATUS_ICON[r.type] ?? null} />
                    </td>
                    <td className="px-3 py-2.5 max-w-xs">
                      <p className="text-[#d0d0d0] truncate" title={r.description}>{r.description}</p>
                    </td>
                    ...
                    <td className="px-3 py-2.5 text-[#a0a0a0] text-xs">{r.technician ?? "—"}</td>
                    <td className="px-3 py-2.5">
                      <Pill label={r.status} cls={STATUS_STYLE[r.status] ?? "text-[#a0a0a0] bg-[#333] border-[#444]"} icon={STATUS_ICON[r.status] ?? null} />
                    </td>
                    <td className="px-3 py-2.5">
      ...
      {/* ── Detail panel ── */}
      {selected && (
        <div className="w-80 flex-shrink-0 rounded-lg border border-[#2a2a2a] bg-[#1a1a1a] overflow-y-auto">
          <div className="p-4 border-b border-[#2a2a2a] flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Wrench className="w-4 h-4 text-amber-400" />
              <span className="text-sm font-medium text-[#f9f9f9]">{selected.id}</span>
            </div>
            <button onClick={() => setSelected(null)} className="text-[#6b7280] hover:text-[#f9f9f9] transition-colors">
          ...
          <div className="p-4 space-y-4 text-sm">
            ...
            <div className="flex flex-wrap gap-2">
              <Pill label={selected.status} cls={STATUS_STYLE[selected.status] ?? "text-[#a0a0a0] bg-[#333] border-[#444]"} icon={STATUS_ICON[selected.status] ?? null} />
              <Pill label={selected.type} cls={TYPE_STYLE[selected.type] ?? "text-[#a0a0a0] bg-[#333] border-[#444]"} icon={STATUS_ICON[selected.type] ?? null} />
            </div>
            ...
            <Section label="Description">
              <p className="text-[#d0d0d0] leading-relaxed">{selected.description}</p>
            </Section>

            {selected.findings && (
              <Section label="Findings / Action Taken">
                <p className="text-[#d0d0d0] leading-relaxed">{selected.findings}</p>
              </Section>
            )}

            <div className="grid grid-cols-2 gap-3">
              <Section label="Date">
                <span className="text-[#d0d0d0]">{selected.date ?? "—"}</span>
              </Section>
              <Section label="Scheduled">
                <span className="text-[#d0d0d0]">{selected.scheduled_date ?? "—"}</span>
              </Section>
              <Section label="Technician">
                <span className="text-[#d0d0d0]">{selected.technician ?? "—"}</span>
              </Section>
